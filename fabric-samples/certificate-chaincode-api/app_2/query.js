...
const query = async (channelName, chaincodeName, args, fcn, username, org_name) => {
    ...
    try {
        ...
        if(fcn === "querySingleGradeSheetbyID"){
            result = await contract.evaluateTransaction(fcn, args[0], args[1], args[2], args[3]);
        } else if(fcn === "queryAllGradeSheetbyID"){
            result = await contract.evaluateTransaction(fcn, args[0], args[1], args[2]);
        } else if(fcn === "queryAllGradeSheet"){
            result = await contract.evaluateTransaction(fcn, args[0]);
        } else if(fcn === "querySingleTranscriptbyID"){
            result = await contract.evaluateTransaction(fcn, args[0], args[1], args[2]);
        } else if(fcn === "queryAllTranscript"){
            result = await contract.evaluateTransaction(fcn, args[0]);
        } else if(fcn === "querySingleCertificatebyID"){
            result = await contract.evaluateTransaction(fcn, args[0], args[1], args[2]);
        } else if(fcn === "queryAllCertificate"){
            result = await contract.evaluateTransaction(fcn, args[0]);
        } else if(fcn === "verify"){
            result = await contract.evaluateTransaction(fcn, args[0], args[1], args[2], args[3]);
        }
